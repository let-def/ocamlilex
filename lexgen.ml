...
let tag_compare t1 t2 = compare t1 t2
...
module StateSet =
  Set.Make (struct type t = t_transition let compare = compare end)


module MemMap =
  Map.Make (struct type t = int let compare = Int.compare end)
...
module MemKey =
  Set.Make
   (struct
     ...
     let compare e1 e2 = match compare e1.tag e2.tag with
